...
// Admin credentials are now managed securely on the server side
// Frontend no longer stores or validates credentials

// Admin Login Form Handler
document.getElementById('adminLoginForm').addEventListener('submit', function(e) {
    // No longer preventing default - form will submit to server
    // e.preventDefault();
    ...
    // Form will now submit to server-side login endpoint
    // Authentication is handled securely on the server
    // Error handling will be done by the server
});

// Authentication is now handled on the server side
// This function is kept for compatibility but no longer performs client-side checks
function checkAdminAuth() {
    // Server-side authentication will handle access control
    // This function is now a placeholder for backward compatibility
}

// Admin logout function now redirects to the server logout endpoint
...
document.addEventListener('DOMContentLoaded', function() {
    // Server-side authentication is now used
    // Username will be populated by the server when rendering the dashboard
});
